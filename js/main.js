...
function displayTopics(topicsArray) {
    ...
    topicsArray.forEach(topic => {
        ...
        // إذا لم يكن هناك subTopics
        if (!topic.subTopics || topic.subTopics.length === 0) {
            // قد يكون الموضوع نفسه له ملف واحد
            if (topic.file) {
                const btn = document.createElement('button');
                btn.textContent = 'Open Topic';
                btn.addEventListener('click', () => {
                    // نمرر اسم الموضوع كعنوان
            ...
            topic.subTopics.forEach(sub => {
                ...
                btn.addEventListener('click', () => {
                    // نمرر عنوان ديناميكي (topicName + sub.name)
...
function loadQuestionsJSON(jsonFilePath, quizTitle) {
    fetch(jsonFilePath)
        ...
        .then(data => {
            ...
            // وضع العنوان الديناميكي
            if (quizTitle) {
                quizTitleElement.textContent = quizTitle;
            } else {
                quizTitleElement.textContent = "Quiz Application";
            }
...
function loadQuiz() {
    ...
    quizData.forEach((data, index) => {
        ...
        if (mode === 'mcq') {
            ...
            // ازرار Show Answer, Shuffle Options, Clear
